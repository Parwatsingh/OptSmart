...
double tTime[3];           //! total time taken by miner & validator.
...
int *mBallotProState;
int *vBallotProState;
int *fvBallotProState;
int *mBallotVotState;
int *vBallotVotState;
int *fvBallotVotState;
int mHBidder;
int mHBid;
int vHBidder;
int vHBid;
	...
	static void concMiner(int t_ID, int numAUs, Graph *cGraph)
	{
		...
		while( curInd < numAUs )
		{
			...
			if (flag == true)
			{
				...
				for(int y = 0; y <= TSRemove; y++) conf_list.remove(y);
				...
				for(auto it = conf_list.begin(); it != conf_list.end(); it++) {
					...
					//! array index start with 0 and index+1 respresent AU_ID.
					int cAUID   = i+1;
...
class ForkValidator
{
	...
	void finalState()
	{
		...
		for(int id = 1; id <= nVoter; id++) 
			ballot->state(id, true, fvBallotVotState);//for voter state

		for(int id = 1; id <= nProposal; id++) 
			ballot->state(id, false, fvBallotProState);//for Proposal state
		
		auction->state(&fvHBidder, &fvHBid); //auction state
	}
...
bool stateVal(bool f) {
	...
	//		cout<<sid<<" \t \t"<<mCoinState[sid]
	//			<<" \t\t"<<vCoinState[sid]<<endl;
			if(mCoinState[sid] != fvCoinState[sid]) flag = true;
		}
	//	cout<<"\n"<<pl<<"Prop ID \tMiner \t\tValidator"<<endl;
		for(int pid = 0; pid < nProposal; pid++) {
	//		cout<<pid+1<<" \t \t"<<mBallotProState[pid]
	//			<<" \t\t"<<vBallotProState[pid]<<endl;
			if(mBallotProState[pid] != fvBallotProState[pid]) flag = true;
		}
	//	cout<<"\n"<<pl<<"Voter ID \tMiner \t\tValidator"<<endl;
		for(int vid = 0; vid < nVoter; vid++) {
	//		cout<<vid+1<<" \t \t"<<mBallotVotState[vid]
	//			<<" \t\t"<<vBallotVotState[vid]<<endl;
			if(mBallotVotState[vid] != fvBallotVotState[vid]) flag = true;
		}
...
{
	cout<<pl<<"MVTO Miner and Concurrent Validator\n";
	cout<<"-----------------------------\n";
		...
		for(int nBlock = 0; nBlock < NumBlock; nBlock++)
		{
			...
			for(int nval = 0; nval < numValidator; nval++)
			{
				...
				for(int p = 0; p < nProposal; p++)
					vBallotProState[p] = fvBallotProState[p] = 0;
				for(int v = 0; v < nVoter; v++)
					vBallotVotState[v] = fvBallotVotState[v] = 0;
				for(int c = 0; c < CoinSObj; c++)
					vCoinState[c] = fvCoinState[c] = 0;
	...
	//		cout<<"\nAvg Grpah Time= "<<gConstT/nThread<<" microseconds";
			...
			int vAUs = gNodeCount;
			if(nBlock > 0)
				file_opr.writeOpt(input, nThread, numAUs, tTime, mTTime, vTTime,
