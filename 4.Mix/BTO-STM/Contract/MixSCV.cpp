...
/*	if(wCountHBidder == 0 && rCountHBidder == 0 
					&& wCountHBid == 0 && rCountHBid == 0)
	{
		wCountHBidder = wCountHBidder + 1;
		wCountHBid    = wCountHBid + 1;
	}
	else
							...
							int *ts, list<int> &cList)
{
	...
	*ts                = txlog->tid; //return time_stamp to caller.
	...
	if( bidValue <= *highestBid )
		return -1;//! invalid AUs: There already a higher bid.

	// If the bid is no longer higher,
	// send the money back to old bidder.
	if (*highestBid != 0) 
	{
...
bool SimpleAuction::AuctionEnded_m( )
{
	...
	*hBider     = 0;
	...
	ops = lib->t_read(txlog, 0, maxBiderID+3, hBid, tb);//! highestBid
	if(ABORT == ops) return false;//AU aborted.
	
	ops = lib->t_read(txlog, 0, maxBiderID+2, hBider, tb);//! highestBidder
	if(ABORT == ops) return false;//AU aborted.
...
void SimpleAuction::state_m(int *hBidder, int *hBid)
{
	...
	*_hBider     = 0;
	...
	ops = lib->t_read(txlog, 0, maxBiderID+3, _hBid, tb);//! highestBid
	ops = lib->t_read(txlog, 0, maxBiderID+2, _hBider, tb);//! highestBidder
...
bool Coin::get_bal_m(int account_iD, int *bal,
						int t_ID, int *time_stamp, list<int>&conf_list) 
{
...
int Ballot::winningProposal_m()
{
	...
	for(int p=(BIDStart+numVoters+1);p<=(BIDStart+numVoters+numPropsals); p++)
	{
		...
		if(ABORT == Pops)
		{
			...
			cout<<"\nError in reading Proposal "
				+to_string(p-numVoters-BIDStart)+"  State.\n";
		}
...
//		cout<<"Proposal = " <<(p-(BIDStart+numVoters))
//			<< " | Vote Count = "<<(*(Proposal*)pObj->val).voteCount;
	}
	txs = lib->bto_TryComit(txlog, conf_list, pObj);
	if(txs == ABORT)cout<<"\nError in reading Winner\n";
	cout<<"\n=======================================================\n";
	cout<<"Winning Proposal = " <<(winningProposal-numVoters-BIDStart)
		<< " | Vote Count = "<<winningVoteCount;
	return winningProposal;
}
...
void Ballot::winnerName_m(string *winnerName)
{
	...
	if(ABORT == Pops)
	{
		cout<<"\nError in reading Winning Proposal "
			+to_string(winningP-numVoters-BIDStart)+"  State.\n";
	}
