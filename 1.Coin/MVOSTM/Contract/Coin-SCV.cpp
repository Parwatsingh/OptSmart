...
//	(*(accNode*)tb->val) = acc;
	...
	*time_stamp = txlog->L_tx_id; //return time_stamp to user.
	...
	if(ABORT != ops)
	{
		string str = "\nAccount "+to_string(receiver_iD)
					+" Old value "+to_string((*(accNode*)tb->val).bal);
		*val += amount;
                 ...
                 int amount, int *time_stamp, list<int>&conf_list) 
{
	...
	*time_stamp = txlog->L_tx_id; //return time_stamp to caller.
	...
	string str = "\nTrans "+to_string(txlog->L_tx_id) +" Sender "
				+to_string(sender_iD)+" balance "
				+to_string((*(accNode*)tb->val).bal);
