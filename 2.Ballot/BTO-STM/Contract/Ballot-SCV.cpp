...
int Ballot::winningProposal_m()
{
	...
	int winningProposal  = 0;
	int winningVoteCount = 0;
	for(int p = numVoters+1 ; p <= numVoters+numProposal ; p++)
	{
	...
	cout<<"\n=======================================================\n";
	cout<<"Winning Proposal = " <<winningProposal-numVoters
	    << " | Vote Count = "<<winningVoteCount;
	return winningProposal;
}
...
void Ballot::winnerName_m(string *winnerName)
{
	...
	if(ABORT == Pops)
		cout<<"\nError in reading Winning Proposal "
	          +to_string(winningP-numVoters)+"  State.\n";
