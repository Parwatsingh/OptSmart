...
#include "Contract/Ballot-SCV.cpp"
...
//		seqTime[1] = 0;
	...
	void finalState()
	{
		for(int id = 1; id <= nVoter; id++) 
			ballot->state(id, true, mVState);//for voter state

		for(int id = 1; id <= nProposal; id++) 
			ballot->state(id, false, mPState);//for Proposal state
	}
	~Miner() { };
};
/********************MINER CODE ENDS*********************************/
...
int main(int argc, char *argv[])
{
	...
	//		printBins( );
			...
			file_opr.writeOpt(nProposal, nVoter, nThread, numAUs, tTime, 
			                  mTTime, vTTime, aCount, vAUs, mItrT, vItrT, 1);
